...
function cratesAPI.lean.new(self, modelpart, minLean, maxLean, speed, enabled)
    local self = setmetatable({}, cratesAPI.lean)
    self.modelpart = modelpart
    if type(minLean) == "table" then
        self.minLean = vec(minLean.x or minLean[1], minLean.y or minLean[2])
    else
        self.minLean = minLean
    end
    if type(maxLean) == "table" then
        self.maxLean = vec(maxLean.x or maxLean[1], maxLean.y or maxLean[2])
    else
...
function cratesAPI:tick()
    ...
    for id_h, v in pairs(head) do
        ...
        if v.enabled then
            ...
            v.selHead = v.vanillaHead and vanilla_model.HEAD or v.modelpart
    ...
    for _, k in pairs(lean) do
        if k.enabled then
            ...
            local t = sin((client.getSystemTime()/50) / 16.0)
            local breathe = vec(
                    t * 2.0,
                    math.sqrt(1 - (t * t)) / 2.0,
                    0.0
                    )
                local lean_x = clamp(sin(-mainrot.x / velmod()) * 45.5, k.minLean.x, k.maxLean.x)
                local lean_y = -clamp(math.sin(mainrot.y) * 45.5, 
                k.minLean.y, k.maxLean.y)
                k.rot = vec(lean_x, lean_y, 0)+breathe
        end
    end
end
...
function cratesAPI:render(delta, ctx)
    ...
    for _, k in pairs(lean) do
        if k.enabled then
             k.modelpart:setOffsetRot(inOutSine(k.modelpart:getOffsetRot(), k.rot, k.speed))
        end
    end
end
